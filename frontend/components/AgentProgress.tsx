...
export default function AgentProgress({ status }: Props) {
  ...
  return (
    <div className="space-y-2.5">
      {AGENTS.map((agent, i) => {
        const { Icon, name, role, description, color } = agent;
        ...
        return (
          <div
            ...
            className={cn(
              ...
              i === 2 && "delay-200",
              i === 3 && "delay-250",
              isDone   && "border-emerald/25 bg-emerald-subtle/30",
              isFailed && "border-destructive/15 bg-destructive/5 opacity-60",
          ...
          >
            ...
            >
              {isDone ? (
                <CheckCircle2 className="h-4.5 w-4.5 text-emerald" strokeWidth={2} />
              ) : isFailed ? (
                <AlertCircle className="h-4.5 w-4.5 text-destructive" strokeWidth={2} />
              ) : isActive ? (
                <Loader2 className={cn("h-4 w-4 animate-spin", color)} strokeWidth={2} />
              ) : (
            ...
            <div className="shrink-0 flex items-center pt-0.5">
              {isDone && (
                <span className="text-xs font-medium text-emerald">Done</span>
              )}
              {isActive && (
            ...
            {/* Progress bar at bottom for active */}
            {isActive && (
      ...
      {/* Status footer */}
      {isActive && (
        <p className="animate-fade-in delay-300 text-center text-xs text-muted-foreground pt-1">
      ...
      {isFailed && (
        <div className="animate-fade-in rounded-xl border border-destructive/20 bg-destructive/5 px-4 py-3">
          <p className="text-sm text-destructive font-medium">Research job failed</p>
